...
class TenantMembershipQueryApiIT {
  ...
  @Test
  void shouldGetPersistedActiveTenantMembership() throws Exception {
    ...
    mockMvc
        .perform(
            get(endpoint(tenant, membership.id().value()))
  ...
  @Test
  void shouldGetPersistedSuspendedTenantMembershipWithoutChangingIt() throws Exception {
  ...
  void shouldReturnNotFoundForUnknownTenant() throws Exception {
    ...
    mockMvc
        .perform(
            get(endpoint(unknownTenantId, unknownMembershipId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + ACCESS_TOKEN)
                .accept(MediaType.APPLICATION_JSON))
  ...
  @Test
  void shouldReturnNotFoundForUnknownMembership() throws Exception {
    ...
    mockMvc
        .perform(
            get(endpoint(tenant, unknownMembershipId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + ACCESS_TOKEN)
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
  ...
  @Test
  void shouldHideMembershipBelongingToAnotherTenant() throws Exception {
    ...
    mockMvc
        .perform(
            get(endpoint(requestedTenant, membership.id().value()))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + ACCESS_TOKEN)
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isNotFound())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
  ...
  private static String endpoint(UUID tenantId, UUID membershipId) {
    return "/api/v1/tenants/" + tenantId + "/memberships/" + membershipId;
  }
