...
class TenantMembershipActivationApiIT {
  ...
  @Test
  void shouldActivateAndPersistSuspendedTenantMembership() throws Exception {
  ...
  @Test
  void shouldRejectAlreadyActiveTenantMembershipWithoutChangingIt() throws Exception {
  ...
  void shouldHideMembershipBelongingToAnotherTenantWithoutChangingIt() throws Exception {
    ...
    TenantMembership membership = TenantMembership.create(owningTenant.id(), platformUser.id());
  ...
  private static String endpoint(UUID tenantId, UUID membershipId) {
    return "/api/v1/tenants/" + tenantId + "/memberships/" + membershipId + "/activation";
  }
