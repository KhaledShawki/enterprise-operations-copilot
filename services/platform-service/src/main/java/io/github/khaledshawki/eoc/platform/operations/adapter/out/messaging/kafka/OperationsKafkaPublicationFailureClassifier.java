...
final class OperationsKafkaPublicationFailureClassifier {
  ...
  private OperationsKafkaPublicationFailureClassifier() {}

  static OperationsEventPublicationException classify(Throwable failure) {
    ...
    if (contains(
      ...
      return new OperationsEventPublicationException(PUBLISH_REJECTED, false, cause);
    }
    if (contains(cause, RetriableException.class)) {
      return new OperationsEventPublicationException(BROKER_RETRYABLE, true, cause);
    }
    if (contains(cause, KafkaException.class)) {
      return new OperationsEventPublicationException(PUBLISH_FAILED, false, cause);
    }
    return new OperationsEventPublicationException(PUBLISH_FAILED, false, cause);
  }
